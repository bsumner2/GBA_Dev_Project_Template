...
#include "tonc_asminc.h"
	...
	.file	"tonc_ISR_Master_ctl.s"
	.extern	_internal_isr_table;

/*! \fn IWRAM_CODE void ISR_Master_ctl()
	\brief	Default irq dispatcher (no automatic nesting)
*/
	@ Register list
	@ r0 : &REG_IE
	@ r1 : _internal_isr_table / isr
	@ r2 : IF & IE
	@ r3 : tmp
	@ ip : (IF,IE)	
BEGIN_FUNC_ARM(ISR_Master_ctl, CSEC_IWRAM)
	@ Read IF/IE 
	mov		r0, #0x04000000
	ldr		ip, [r0, #0x200]!
...
.Lpost_search:
	ldrne	r1, [r1, #-4]		@ isr= _internal_isr_table[ii-1].isr
	cmpne	r1, #0
